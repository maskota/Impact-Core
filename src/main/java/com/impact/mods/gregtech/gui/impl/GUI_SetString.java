...
import com.impact.network.packets.ImpactPacketSetString;
...
public class GUI_SetString extends GT_GUIContainerMetaTile_Machine {

    private GuiTextField gui;
    private GuiButton apply;

    public GUI_SetString(InventoryPlayer aInventoryPlayer, IGregTechTileEntity aTileEntity) {
        super(new Container_SetString(aInventoryPlayer, aTileEntity), RES_PATH_GUI + "Aerostat.png");
    }
    ...
    protected final void drawGuiContainerBackgroundLayer(float f, int x, int y) {
        ...
        this.fontRendererObj.drawString("Set name:", guiLeft + 28, guiTop + 8, 0);
        try {
    ...
    protected void actionPerformed(GuiButton btn) {
        if (btn == apply) {
            ImpactNetwork.INSTANCE.sendToServer(new ImpactPacketSetString(gui.getText(), Minecraft.getMinecraft().thePlayer));
